...
The same pipeline runs in the classic CLI, the TUI, and the desktop app (which streams PCM from its backend over a dedicated WebSocket instead of waiting for a whole audio file).
