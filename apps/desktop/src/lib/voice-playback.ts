...
/**
 * Play `text` over the streaming WS. Resolves 'played' once audio finished
 * (or was stopped/barged), 'fallback' when streaming never produced audio and
 * the caller should use the synchronous POST path instead.
 */
function playSpeechStream(wsUrl: string, text: string, options: VoicePlaybackOptions): Promise<'fallback' | 'played'> {
  return new Promise(resolve => {
    ...
    const settle = (value: 'fallback' | 'played') => {
      if (!settled) {
        settled = true
        teardown()
        resolve(value)
    ...
    ws.onmessage = event => {
      ...
      if (frame.type === 'start') {
        ...
        settle(started ? 'played' : 'fallback')
      }
    }
    ...
    ws.onerror = () => settle(started ? 'played' : 'fallback')
