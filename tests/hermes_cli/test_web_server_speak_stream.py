...
def test_rejects_empty_text(stream_client):
    with stream_client.websocket_connect(_url()) as conn:
        conn.send_text(json.dumps({"text": "   "}))
        with pytest.raises(WebSocketDisconnect) as exc:
            conn.receive_json()
...
def test_fallback_frame_when_no_streaming_provider(stream_client, monkeypatch):
    ...
    with stream_client.websocket_connect(_url()) as conn:
        conn.send_text(json.dumps({"text": "Hello there."}))
...
def test_long_text_is_split_across_provider_requests(stream_client, monkeypatch):
    ...
    with stream_client.websocket_connect(_url()) as conn:
        conn.send_text(json.dumps({"text": "First sentence here. Second sentence here. Third one."}))
